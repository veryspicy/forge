  ...
  <div class="flex flex-col gap-4">
    <NSpin :show="loading">
      <template v-if="order">
        ...
        <NSpace class="mb-4 flex-wrap">
          <NButton v-if="order.status === 'PAID'" type="success" @click="openReview(true)">{{ $t('page.ordersDetail.approve') }}</NButton>
          <NButton v-if="order.status === 'PAID'" type="error" @click="openReview(false)">{{ $t('page.ordersDetail.reject') }}</NButton>
          <NButton v-if="order.status === 'PROCESSING'" type="primary" @click="openProcure()">{{ $t('page.ordersDetail.pushToProcurement') }}</NButton>
          <NButton v-if="order.status === 'PROCURE_FAILED'" type="error" @click="openProcure()">{{ $t('page.ordersDetail.retryProcurement') }}</NButton>
          <NButton @click="openRefund()">{{ $t('page.ordersDetail.refund') }}</NButton>
          <NButton v-if="order.status === 'PROCURING'" type="primary" @click="openShip()">{{ $t('page.ordersDetail.ship') }}</NButton>
        </NSpace>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <NCard :title="$t('page.ordersDetail.orderInfo')" size="small">
            <div class="flex flex-col gap-2 text-sm">
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.orders.orderNumber') }} </span>{{ order.order_number }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.userId') }} </span>{{ order.user_id }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.status') }} </span><NTag :type="statusType(order.status)" size="small">{{ order.status }}</NTag></div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.orders.total') }} </span>${{ order.total }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.settings.defaultCurrency') }} </span>{{ order.currency }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.created') }} </span>{{ order.created_at }}</div>
            </div>
          </NCard>
          ...
          <NCard v-if="order.shipping_address" :title="$t('page.ordersDetail.shippingAddress')" size="small">
            <div class="flex flex-col gap-2 text-sm">
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.name') }} </span>{{ order.shipping_address.name }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.addressLine1') }} </span>{{ order.shipping_address.line1 }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.city') }} </span>{{ order.shipping_address.city }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.state') }} </span>{{ order.shipping_address.state }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.country') }} </span>{{ order.shipping_address.country }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.phone') }} </span>{{ order.shipping_address.phone }}</div>
            </div>
          </NCard>
          ...
          <NCard v-if="order.review_status" :title="$t('page.ordersDetail.reviewStatus')" size="small">
            <div class="flex flex-col gap-2 text-sm">
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.reviewedBy') }} </span>{{ order.review_status.reviewed_by }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.approved') }} </span>{{ order.review_status.approved }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.reason') }} </span>{{ order.review_status.reason || '-' }}</div>
            </div>
          </NCard>
          ...
          <NCard v-if="order.procurement_info" :title="$t('page.ordersDetail.procurementInfo')" size="small">
            <div class="flex flex-col gap-2 text-sm">
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.ordersDetail.supplier') }} </span>{{ order.procurement_info.supplier_id }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.sku') }} </span>{{ order.procurement_info.supplier_sku }}</div>
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('common.cost') }} </span>${{ order.procurement_info.cost }}</div>
            </div>
          </NCard>
          ...
          <NCard v-if="order.tracking_number" :title="$t('page.ordersDetail.shipping')" size="small">
            <div class="flex flex-col gap-2 text-sm">
              <div><span class="text-[var(--n-text-color-3)]">{{ $t('page.shipments.trackingNumber') }} </span>{{ order.tracking_number }}</div>
            </div>
          </NCard>
        </div>
      </template>
    ...
    <NModal v-model:show="showReview" preset="card" :title="$t('page.ordersDetail.approveOrder')" style="width:440px">
      <div class="flex flex-col gap-3">
        <NFormItem :label="$t('page.ordersDetail.reason')"><NInput v-model:value="reviewReason" type="textarea" :rows="2" /></NFormItem>
    ...
    <!-- Procure Modal -->
    <NModal v-model:show="showProcure" preset="card" :title="$t('page.ordersDetail.pushToProcurement')" style="width:440px">
      <div class="flex flex-col gap-3">
        <NFormItem :label="$t('page.ordersDetail.supplierId')" required><NInput v-model:value="procureSupplierId" /></NFormItem>
        <NFormItem :label="$t('page.ordersDetail.supplierSku')"><NInput v-model:value="procureSku" /></NFormItem>
        <NFormItem :label="$t('common.cost')"><NInputNumber v-model:value="procureCost" :min="0" :step="0.01" style="width:100%" /></NFormItem>
        <div v-if="actionError" class="text-red-500 text-sm">{{ actionError }}</div>
      </div>
      <template #footer>
        <NSpace justify="end">
    ...
    <NModal v-model:show="showShip" preset="card" :title="$t('page.ordersDetail.shipOrder')" style="width:440px">
      <div class="flex flex-col gap-3">
        <NFormItem :label="$t('page.shipments.trackingNumber')" required><NInput v-model:value="shipTracking" /></NFormItem>
    ...
    <NModal v-model:show="showRefund" preset="card" :title="$t('page.ordersDetail.refundOrder')" style="width:440px">
      <div class="flex flex-col gap-3">
        <NFormItem :label="$t('page.ordersDetail.reason')"><NInput v-model:value="refundReason" type="textarea" :rows="2" /></NFormItem>
        <div v-if="actionError" class="text-red-500 text-sm">{{ actionError }}</div>
      </div>
      <template #footer>
        <NSpace justify="end">
          <NButton @click="showRefund = false">{{ $t('common.cancel') }}</NButton>
          <NButton type="error" :loading="actionLoading" @click="doRefund">{{ $t('page.ordersDetail.confirmRefund') }}</NButton>
        </NSpace>
      </template>
    </NModal>
  </div>
