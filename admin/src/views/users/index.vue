...
const columns: DataTableColumns<any> = [
  ...
  {
    title: t('common.enforce'),
    key: 'role_enforced',
    render: row => (row.role_enforced ? t('common.yes') : t('common.no'))
  },
  { title: t('common.created'), key: 'created_at', render: row => formatDate(row.created_at) },
  {
    ...
    key: 'actions',
    render: row =>
      isAdmin.value && row.id !== currentUser.value?.id
        ? h(
            NButton,
            { size: 'small', onClick: () => openRoleModal(row) },
            { default: () => t('page.users.changeRole') }
          )
        : null
  }
];
  ...
  <div class="flex flex-col gap-4">
    ...
    >
      <div class="flex flex-col gap-3">
        <div>
          <span class="text-[var(--n-text-color-3)]">User:</span>
          {{ roleUser?.email }}
        </div>
        <div>
