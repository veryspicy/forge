...
import { NButton, NColorPicker, NInput, NInputNumber, NSelect, NSwitch, NTag } from 'naive-ui';
...
function sanitizeSvg(input: string): string {
  ...
  s = s.replace(
    /(\s(?:href|src|srcdoc|xlink:href|action|style)\s*=\s*)("([^"]*)"|'([^']*)')/gi,
    (match, _attr, quoted) => {
  ...
  s = s.replace(/(\sstyle\s*=\s*)("([^"]*)"|'([^']*)')/gi, (_, attr, quoted) => {
    ...
    const cleaned = raw.replace(/expression\s*\(/gi, '').replace(/url\s*\(\s*['"]?\s*javascript:/gi, 'url("');
    return `${attr}"${cleaned}"`;
  });
  return s;
}
...
const localeTabsOptions = computed(() => {
  ...
  return locales.filter(Boolean).map((locale: string) => {
...
function renameTranslationKey(oldKey: string, newKey: string, fallbackValue?: string) {
  ...
  for (const locale of locales) {
    ...
    if (from && Object.prototype.hasOwnProperty.call(dict, from)) {
      dict[to] = dict[from];
      const { [from]: _dropped, ...rest } = dict;
      Object.assign(dict, rest);
...
function onHeroTextKeyChanged(
  field: 'titleKey' | 'subtitleKey' | 'cta1LabelKey' | 'cta2LabelKey',
  oldKey: string,
  v: string | number
) {
...
function deleteCustomFlag(key: string) {
  if (config.value.featureFlags) {
    const { [key]: _dropped, ...rest } = config.value.featureFlags;
    Object.assign(config.value.featureFlags, rest);
  }
}
...
watch(
  () => config.value.i18n?.defaultLocale,
  v => {
...
function updateTranslationKey(idx: number, newKey: string) {
  ...
  const { [oldKey]: _dropped, ...rest } = config.value.i18n.translations[locale];
  Object.assign(config.value.i18n.translations[locale], rest);
...
function removeTranslation(k: string) {
  const locale = activeLocale.value;
  if (config.value.i18n?.translations?.[locale]) {
    const { [k]: _dropped, ...rest } = config.value.i18n.translations[locale];
    Object.assign(config.value.i18n.translations[locale], rest);
  }
}
</script>
...
<template>
  <div class="property-panel h-full overflow-y-auto rounded bg-white p-4 shadow-sm dark:bg-dark">
    <template v-if="selectedElement">
        ...
        <div v-if="selectedElement.classes.length" class="flex items-start gap-2">
          ...
          <div class="flex flex-wrap gap-1">
            <span
              ...
              :key="cls"
              class="font-mono rounded bg-blue-100 px-1 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300"
            >
              .{{ cls }}
            </span>
          </div>
      ...
      <div
        class="rounded border border-dashed border-gray-200 border-solid p-3 text-xs text-gray-400 dark:border-gray-700"
      >
    ...
    <template v-else-if="activeConfigKey">
      <div class="mb-4 flex items-center gap-2 border-b border-gray-100 border-solid pb-3 dark:border-gray-700">
        ...
        <NButton size="tiny" type="primary" class="ml-auto" :loading="saving" @click="handleSaveSiteConfig">
          保存站点配置
        </NButton>
      </div>
      ...
      <div v-if="activeConfigKey === 'brand'" class="flex flex-col gap-3">
        ...
        <ConfigCard title="品牌预览">
          <div class="flex items-center gap-3">
            <div class="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden">
              ...
              >
                <img :src="config.brand.logo.data" class="max-h-full max-w-full object-contain" alt="Logo" />
              </div>
              <div
        ...
        <FieldLabel
          ...
          type="string"
          range="≤ 120 字符"
          example="AI 驱动的宠物用品商店"
          desc="一句话品牌介绍（可留空），显示在页头副标题区域。"
        />
        <NInput v-model:value="config.brand.tagline" placeholder="一句话品牌介绍（可留空）" />
        <FieldLabel
          label="品牌文字颜色"
          name="brand.nameColor"
          type="color"
          range="auto | #hex"
          example="#18a058"
          desc="auto=跟随主题主色（主题变化时品牌文字颜色随之变化）；或选择自定义固定颜色。C 端品牌名称/文字 Logo 与预览一致。"
        />
        <div class="flex items-center gap-2">
          <NSelect
        ...
        <FieldLabel
          ...
          name="logo.type"
          type="enum"
          range="text | image | svg"
          example="text"
          desc="Logo 渲染方式：文字 / 图片 / 内联 SVG 代码。"
        />
        <NSelect
        ...
        <template v-else-if="config.brand.logo.type === 'image'">
          <FieldLabel
            ...
            name="logo.data"
            type="image URL"
            range="http(s):// 或 data:image"
            example="https://example.com/logo.png"
            desc="上传 Logo 图片，或手动输入图片 URL。"
          />
          ...
          <div
            ...
            class="flex items-center gap-2 rounded border border-gray-200 border-solid bg-gray-50 p-2"
          >
        ...
        <template v-else-if="config.brand.logo.type === 'svg'">
          ...
          />
          <div
            class="flex flex-col gap-2 rounded border border-dashed border-gray-200 border-solid bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800"
          >
            ...
            >
              <div class="h-full w-auto [&>svg]:h-full [&>svg]:w-auto [&>svg]:max-h-full" v-html="brandSvgPreview" />
            </div>
            <div
              v-else
      ...
      <div v-else-if="activeConfigKey === 'theme'" class="flex flex-col gap-4">
        <FieldLabel
          ...
          name="preset"
          type="enum"
          range="forge | apple | cloudflare | linear | vercel | stripe | notik | shopify"
          example="forge"
          desc="一键套用预置配色方案，点击后覆盖下方各色值。"
        />
        <div class="grid grid-cols-2 gap-2">
          ...
          >
            <div class="mb-1 flex items-center gap-1">
              <div
                v-for="(color, ci) in preset.swatch"
                :key="ci"
        ...
        <div class="flex flex-col gap-3">
          <FieldLabel
            ...
            range="#000000 ~ #ffffff"
            example="#18a058"
            desc="主题主色，用于按钮/链接/选中态等核心交互元素。"
          />
          <div class="flex items-center gap-2">
            <NColorPicker
              :value="config.theme.primaryColor"
              size="small"
              @update:value="v => (config.theme.primaryColor = v)"
            />
          ...
          <FieldLabel
            ...
            range="#000000 ~ #ffffff"
            example="#36ad6a"
            desc="主色的浅色变体，用于 hover 状态 / 浅色背景。"
          />
          <div class="flex items-center gap-2">
            <NColorPicker
              :value="config.theme.primaryLight"
              size="small"
              @update:value="v => (config.theme.primaryLight = v)"
            />
          ...
          <FieldLabel
            ...
            range="#000000 ~ #ffffff"
            example="#0c7a43"
            desc="主色的深色变体，用于 active 状态 / 强调文字。"
          />
          <div class="flex items-center gap-2">
            <NColorPicker
              :value="config.theme.primaryDark"
              size="small"
              @update:value="v => (config.theme.primaryDark = v)"
            />
          ...
          <FieldLabel
            ...
            range="#000000 ~ #ffffff"
            example="#f0a020"
            desc="辅助色，用于次要按钮 / 标签 / 价格等。"
          />
          <div class="flex items-center gap-2">
            <NColorPicker
              :value="config.theme.secondaryColor"
              size="small"
              @update:value="v => (config.theme.secondaryColor = v)"
            />
          ...
          <FieldLabel
            ...
            range="#000000 ~ #ffffff"
            example="#2080f0"
            desc="强调色，用于促销/提示/链接等需要突出的元素。"
          />
          <div class="flex items-center gap-2">
            <NColorPicker
              :value="config.theme.accentColor"
              size="small"
              @update:value="v => (config.theme.accentColor = v)"
            />
          ...
          <FieldLabel
            ...
            range="CSS font-family 值"
            example="Inter, sans-serif"
            desc="标题字体栈，可填 Google Fonts 字体名 + 兜底字体。"
          />
          ...
          <FieldLabel
            ...
            range="CSS font-family 值"
            example="Inter, sans-serif"
            desc="正文字体栈，可填 Google Fonts 字体名 + 兜底字体。"
          />
      ...
      <div v-else-if="activeConfigKey === 'navigation'" class="flex flex-col gap-3">
        <!-- 快捷预设按钮 -->
        <div
          class="flex flex-wrap items-center gap-1 rounded border border-dashed border-gray-200 border-solid bg-gray-50 p-2 dark:border-gray-700 dark:bg-gray-800"
        >
        ...
        <div
          class="rounded border border-dashed border-gray-200 border-solid p-2 text-[11px] leading-relaxed text-gray-400 dark:border-gray-700"
        >
          🔑 公共前缀：导航翻译 key 统一以
          <b>nav.</b>
          开头，下方每个导航项的「翻译 Key 后缀」只需填写后半段，保存时自动拼接（如
          <code>myPets</code>
          →
          <code>nav.myPets</code>
          ）。
        </div>

        <template v-if="config.navigation.length === 0">
          <div
            class="rounded border border-dashed border-gray-200 border-solid p-6 text-center text-sm text-gray-400 dark:border-gray-700"
          >
          ...
          >
            <template #header>
              <SvgIcon
                ...
                class="nav-drag-handle cursor-grab shrink-0 text-gray-400 hover:text-gray-600"
              />
              ...
              <NButton size="tiny" quaternary type="error" class="shrink-0" @click="config.navigation.splice(idx, 1)">
                <template #icon><SvgIcon icon="mdi:close" /></template>
              </NButton>
            </template>
            ...
            <div v-if="navItem._open !== false" class="flex flex-col gap-3">
              ...
              <div class="flex flex-col gap-1">
                <FieldLabel
                  ...
                  range="自动拼接 nav. 前缀"
                  example="myPets"
                  desc="导航 i18n key 统一以 nav. 开头（公共前缀已置顶展示），此处只填后缀，保存时自动拼接为 nav.myPets；留空则按名称自动生成。"
                />
                <div class="flex items-center gap-1">
                  <NTag
                    ...
                    :bordered="false"
                    class="shrink-0"
                    title="导航 i18n key 公共前缀，自动拼接"
                  >
                    nav.
                  </NTag>
                  <NInput
                    :value="navKeySuffix(navItem.labelKey)"
                    size="small"
                    placeholder="如 myPets（留空将自动生成）"
                    @update:value="v => onNavLabelKeyChanged(idx, navItem.labelKey, v)"
                  />
                </div>
              </div>
              <div class="flex flex-col gap-1">
                <FieldLabel
                  label="联动功能开关"
                  name="featureFlag"
                  type="string (flag key)"
                  range="功能开关 key 或留空"
                  example="show_ai_chat"
                  desc="关联功能开关：开关关闭时自动隐藏该导航项；留空表示不联动。"
                />
                <NSelect
        ...
        <div
          class="rounded border border-dashed border-gray-200 border-solid p-2 text-[11px] leading-relaxed text-gray-400 dark:border-gray-700"
        >
          💡 提示：翻译 key 自动拼接 nav. 前缀；修改「名称」将自动生成唯一后缀并回填 zh/en 翻译；关联 feature flag
          后，开关关闭会自动隐藏此导航项。
        </div>
      </div>
        ...
        >
          <ConfigCard v-for="(cat, idx) in config.categories" :key="cat.slug || idx">
            <template #header>
              <SvgIcon
                icon="mdi:drag-vertical"
                class="cat-drag-handle cursor-grab shrink-0 text-gray-400 hover:text-gray-600"
              />
            ...
            <div class="flex items-start gap-3">
              <div class="flex w-16 shrink-0 flex-col items-center gap-1">
                <div
                  v-if="!cat.image"
                  class="flex h-16 w-16 cursor-pointer items-center justify-center rounded border border-dashed border-gray-300 border-solid text-xs text-gray-400 hover:border-gray-400 dark:border-gray-600"
                  @click="openResourcePicker({ target: 'categoryImage', index: Number(idx) })"
                >
                ...
                <div v-else class="relative h-16 w-16 shrink-0">
                  ...
                  <NButton
                    ...
                    quaternary
                    type="error"
                    class="absolute -right-1 -top-1 !p-0 !h-4 !w-4"
                    @click="cat.image = ''"
                  >
              ...
              <div class="flex min-w-0 flex-1 flex-col gap-2">
                <div class="flex flex-col gap-1">
                  <FieldLabel
                    ...
                    type="image URL"
                    range="http(s):// 或上传"
                    example="https://cdn.example.com/cat-food.png"
                    desc="分类卡片展示图；未上传时可用 Emoji 图标替代。"
                  />
                ...
                <div class="flex flex-col gap-1">
                  <FieldLabel
                    ...
                    range="点分命名，留空自动生成"
                    example="cat.food"
                    desc="分类名称的 i18n 翻译 key；修改名称时自动生成并回填翻译。"
                  />
                  <div class="flex items-center gap-1">
                    <NTag
                      size="small"
                      ...
                      :bordered="false"
                      class="shrink-0"
                      title="分类 i18n key 公共前缀，自动拼接"
                    >
                      cat.
                    </NTag>
                    <NInput
                      :value="catKeySuffix(cat.nameKey)"
                      size="small"
                      placeholder="如 food（留空自动生成）"
                      @update:value="v => onCategoryNameKeyChanged(idx, cat.nameKey, v)"
                    />
                  </div>
                </div>
                <div class="flex flex-col gap-1">
      ...
      <div v-else-if="activeConfigKey === 'footer'" class="flex flex-col gap-4">
        ...
        <ConfigCard title="社交媒体">
          <div class="flex flex-col gap-2">
            ...
            />
            <div
              v-for="(item, sIdx) in config.footer.social || []"
              :key="item.platform || sIdx"
              class="flex items-center gap-2 rounded border border-gray-200 border-solid bg-white px-2 py-1.5 dark:border-gray-700 dark:bg-dark"
            >
              <span class="w-20 shrink-0 text-xs font-medium capitalize text-gray-600 dark:text-gray-300">
                {{ item.platform }}
              </span>
        ...
        <VueDraggable
          v-model="config.footer.linkGroups"
        ...
        >
          <ConfigCard v-for="(group, gIdx) in config.footer.linkGroups" :key="group.key || gIdx">
            <template #header>
              <SvgIcon
                icon="mdi:drag-vertical"
                class="group-drag-handle cursor-grab shrink-0 text-gray-400 hover:text-gray-600"
              />
              ...
              <NButton
                ...
                type="error"
                class="shrink-0"
                @click="config.footer.linkGroups.splice(gIdx, 1)"
              >
                <template #icon><SvgIcon icon="mdi:close" /></template>
              </NButton>
            </template>
            ...
            <div class="flex flex-col gap-3">
              ...
              <div class="flex flex-col gap-1">
                <FieldLabel
                  ...
                  name="titleKey"
                  type="string"
                  range="自动拼接 footer. 前缀"
                  example="support"
                  desc="分组标题翻译 key 统一以 footer. 开头，此处只填后缀，保存时自动拼接为 footer.support；留空则按标题自动生成。"
                />
                <div class="flex items-center gap-1">
                  <NTag
                    size="small"
                    ...
                    :bordered="false"
                    class="shrink-0"
                    title="页脚分组 i18n key 公共前缀，自动拼接"
                  >
                    footer.
                  </NTag>
                  <NInput
                    :value="footerKeySuffix(group.titleKey)"
                    size="small"
                    placeholder="如 support（留空自动生成）"
                    @update:value="v => onGroupTitleKeyChanged(gIdx, group.titleKey, v)"
                  />
                </div>
              </div>
              ...
              <div class="flex flex-col gap-2">
                <div
                  ...
                  :key="lIdx"
                  class="rounded border border-gray-200 border-solid bg-white dark:border-gray-700 dark:bg-dark"
                >
                  <!-- 第一行：开关 + 链接文字 + 删除 -->
                  <div
                    class="flex items-center gap-2 border-b border-gray-100 border-solid px-2 py-1.5 dark:border-gray-700"
                  >
                  ...
                  <div class="flex flex-col gap-2 p-2">
                    <div class="flex flex-col gap-1">
                      <FieldLabel
                        ...
                        range="自动拼接 footer. 前缀"
                        example="faqs"
                        desc="链接翻译 key 统一以 footer. 开头，此处只填后缀，保存时自动拼接为 footer.faqs；留空则按链接文字自动生成。"
                      />
                      <div class="flex items-center gap-1">
                        <NTag
                          size="small"
                          ...
                          :bordered="false"
                          class="shrink-0"
                          title="页脚链接 i18n key 公共前缀，自动拼接"
                        >
                          footer.
                        </NTag>
                        <NInput
                          :value="footerKeySuffix(link.labelKey)"
                          size="small"
                          placeholder="如 faqs（留空自动生成）"
                          @update:value="v => onLinkLabelKeyChanged(gIdx, lIdx, link.labelKey, v)"
                        />
                      </div>
                    </div>
                    <div class="flex flex-col gap-1">
      ...
      <div v-else-if="activeConfigKey === 'homeHero'" class="flex flex-col gap-4">
        <ConfigCard title="展示模式">
          <div class="flex items-center justify-between">
            <FieldLabel
              ...
              range="true | false"
              example="false"
              desc="开启后使用轮播模式（多图自动播放），关闭则为单张 Hero 大图。"
            />
        ...
        <template v-if="!config.homeHero.useCarousel">
          <FieldLabel
            ...
            type="string"
            range="≤ 60 字符"
            example="Smart Shopping for Your Pet"
            desc="Hero 大标题文案。"
          />
          ...
          <div class="flex flex-col gap-1">
            <FieldLabel
              ...
              range="自动拼接 hero. 前缀"
              example="title"
              desc="Hero 文案 i18n key 统一以 hero. 开头，此处只填后缀，保存时自动拼接为 hero.title。"
            />
            <div class="flex items-center gap-1">
              <NTag
                size="small"
                ...
                :bordered="false"
                class="shrink-0"
                title="Hero i18n key 公共前缀，自动拼接"
              >
                hero.
              </NTag>
              <NInput
                :value="heroKeySuffix(config.homeHero.hero.titleKey)"
                size="small"
                placeholder="如 title（留空自动生成）"
                @update:value="v => onHeroTextKeyChanged('titleKey', config.homeHero.hero.titleKey, v)"
              />
            </div>
          </div>
          <FieldLabel
          ...
          <div class="flex flex-col gap-1">
            <FieldLabel
              label="副标题翻译 Key 后缀"
              name="hero.subtitleKey"
              type="string"
              range="自动拼接 hero. 前缀"
              example="subtitle"
              desc="Hero 副标题 i18n key 统一以 hero. 开头，此处只填后缀，保存时自动拼接为 hero.subtitle。"
            />
            <div class="flex items-center gap-1">
              <NTag
                size="small"
              ...
              <NInput
                :value="heroKeySuffix(config.homeHero.hero.subtitleKey)"
                size="small"
                placeholder="如 subtitle（留空自动生成）"
                @update:value="v => onHeroTextKeyChanged('subtitleKey', config.homeHero.hero.subtitleKey, v)"
              />
            </div>
          </div>
          ...
          <div class="flex flex-col gap-1">
            <FieldLabel
              label="主按钮翻译 Key 后缀"
              name="hero.cta1LabelKey"
              type="string"
              range="自动拼接 hero. 前缀"
              example="cta1Label"
              desc="主按钮文案 i18n key 统一以 hero. 开头，此处只填后缀，保存时自动拼接为 hero.cta1Label。"
            />
            <div class="flex items-center gap-1">
              <NTag
                size="small"
              ...
              <NInput
                :value="heroKeySuffix(config.homeHero.hero.cta1LabelKey)"
                size="small"
                placeholder="如 cta1Label（留空自动生成）"
                @update:value="v => onHeroTextKeyChanged('cta1LabelKey', config.homeHero.hero.cta1LabelKey, v)"
              />
            </div>
          </div>
          <FieldLabel
          ...
          <div class="flex flex-col gap-1">
            <FieldLabel
              label="次按钮翻译 Key 后缀"
              name="hero.cta2LabelKey"
              type="string"
              range="自动拼接 hero. 前缀"
              example="cta2Label"
              desc="次按钮文案 i18n key 统一以 hero. 开头，此处只填后缀，保存时自动拼接为 hero.cta2Label。"
            />
            <div class="flex items-center gap-1">
              <NTag
                size="small"
              ...
              <NInput
                :value="heroKeySuffix(config.homeHero.hero.cta2LabelKey)"
                size="small"
                placeholder="如 cta2Label（留空自动生成）"
                @update:value="v => onHeroTextKeyChanged('cta2LabelKey', config.homeHero.hero.cta2LabelKey, v)"
              />
            </div>
          </div>
          <FieldLabel
          ...
          <div v-if="config.homeHero.hero.backgroundImage" class="relative">
            ...
            <NButton
              ...
              @click="config.homeHero.hero.backgroundImage = ''"
            >
              <template #icon><SvgIcon icon="mdi:close" /></template>
            </NButton>
          </div>
        ...
        <template v-else>
          <ConfigCard title="轮播设置">
            <div class="flex flex-col gap-3">
              ...
              <div class="flex flex-col gap-1">
                ...
                />
                <NInputNumber
                  v-model:value="config.homeHero.carousel.interval"
                  :min="500"
                  :step="500"
                  size="small"
                  style="width: 120px"
          ...
          <FieldLabel
            ...
            type="array<{url,alt,link,title}>"
            range="url 必填"
            example="url: https://.../banner.png"
            desc="轮播图片列表：每张图可配置标题、Alt 文本与跳转链接。"
          />
          <VueDraggable
            v-model="config.homeHero.carousel.images"
            ...
            <ConfigCard v-for="(img, iIdx) in config.homeHero.carousel.images" :key="iIdx">
              <div class="flex items-start gap-3">
                <div class="flex w-6 shrink-0 items-center justify-center self-center">
                  <SvgIcon
                    icon="mdi:drag-vertical"
                    class="carousel-drag-handle cursor-grab text-gray-400 hover:text-gray-600"
                  />
                </div>
                <div class="w-24 shrink-0">
                  <div
                    v-if="!img.url"
                    class="flex h-16 w-24 cursor-pointer items-center justify-center rounded border border-dashed border-gray-300 border-solid text-xs text-gray-400 hover:border-gray-400 dark:border-gray-600"
                    @click="openResourcePicker({ target: 'carouselImage', index: Number(iIdx) })"
                  >
                    <SvgIcon icon="mdi:upload" />
                  </div>
                  <div v-else class="relative">
                    ...
                    <div
                      class="absolute inset-0 flex cursor-pointer items-center justify-center rounded bg-black/30 opacity-0 transition-opacity hover:opacity-100"
                      @click="openResourcePicker({ target: 'carouselImage', index: Number(iIdx) })"
                    >
                ...
                <NButton
                  ...
                  type="error"
                  class="shrink-0"
                  @click="config.homeHero.carousel.images.splice(iIdx, 1)"
                >
                  <template #icon><SvgIcon icon="mdi:close" /></template>
                </NButton>
              </div>
          ...
          <NButton
            ...
            dashed
            style="width: 100%"
            @click="openResourcePicker({ target: 'carouselImageNew' }, true)"
          >
            <template #icon><SvgIcon icon="mdi:plus" /></template>
            添加轮播图片
          </NButton>
      ...
      <div v-else-if="activeConfigKey === 'seo'" class="flex flex-col gap-3">
        ...
        <FieldLabel
          ...
          type="string"
          range="≤ 150 字符"
          example="AI 驱动的宠物用品商城"
          desc="首页 Meta 描述，展示在搜索结果摘要。"
        />
      ...
      <div v-else-if="activeConfigKey === 'i18n'" class="flex flex-col gap-4">
        ...
        <template v-else>
          ...
          <div class="flex max-h-96 flex-col gap-2 overflow-y-auto">
            <ConfigCard v-for="entry in translationEntries" :key="entry._idx" content-padding="8px 12px">
              <div class="flex items-center gap-2">
                <NInput
                  ...
                  class="font-mono"
                  placeholder="Key"
                  @update:value="v => updateTranslationKey(entry._idx, v)"
                />
                <NInput
                  :value="entry.v"
                  size="small"
                  placeholder="翻译值"
                  @update:value="v => updateTranslationVal(entry._idx, v)"
                />
            ...
            <div
              v-if="Object.keys(currentTranslations).length === 0"
              class="rounded border border-dashed border-gray-200 border-solid p-6 text-center text-xs text-gray-400 dark:border-gray-700"
            >
              暂无翻译条目，使用下方添加
            </div>
          </div>
      ...
      <div v-else-if="activeConfigKey === 'featureFlags'" class="flex flex-col gap-3">
        <FieldLabel
          ...
          type="boolean"
          range="true | false"
          example="show_ai_chat: true"
          desc="内置功能开关；关闭后对应功能在前台隐藏。导航项可通过 featureFlag 关联联动显隐。"
        />
        ...
        <FieldLabel
          label="自定义功能开关"
          name="featureFlags.<自定义key>"
          type="boolean"
          range="小写字母/数字/下划线"
          example="enable_loyalty"
          desc="自定义功能开关；输入 key 后添加，供前台按 key 判断。"
        />
        ...
        <div class="flex items-center gap-2 pt-1">
          <NInput
            v-model:value="newFlagKey"
            size="small"
            placeholder="自定义开关 key (如 enable_xxx)"
            style="flex: 1; min-width: 0"
          />
      ...
      <div v-else-if="activeConfigKey === 'products'" class="flex flex-col gap-3">
        <FieldLabel
          label="商品默认排序"
          name="products.default_sort"
          type="string"
          range="default | sales | newest | price_asc | price_desc"
          example="default"
          desc="C 端商品列表默认排序档；default 为综合排序（人工权重 sort_order 优先 → 销量 → 上架时间）。"
        />
      ...
      <div v-else-if="activeConfigKey === 'currencies'" class="flex flex-col gap-2">
        <FieldLabel
          label="结算货币"
          name="currencies[]"
          type="array<string (ISO 4217)>"
          range="3 位大写字母代码"
          example="['CNY','USD']"
          desc="启用的结算货币列表；第一个为默认展示货币。"
        />
