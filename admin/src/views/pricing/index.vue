...
function updatePromoRegions() {
  promoForm.value.applicable_regions = promoRegions.value
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
}
function updatePromoCats() {
  promoForm.value.applicable_categories = promoCategories.value
    .split(',')
    .map(s => s.trim())
    .filter(Boolean);
}
...
const ruleColumns: DataTableColumns<any> = [
  ...
  { title: t('common.region'), key: 'region' },
      ...
      h(
        ...
        { size: 'small' },
        {
          default: () => [
            ...
            h(
              NButton,
              { size: 'small', type: 'error', onClick: () => deletePromo(row.id) },
              { default: () => t('common.delete') }
            )
          ]
        }
...
async function fetchRules() {
  const res = await get('/api/admin/v1/pricing/rules');
  rules.value = res.data || [];
}
async function fetchPromos() {
  const res = await get('/api/admin/v1/pricing/promotions');
...
function openRuleModal(r?: any) {
  ...
  ruleForm.value = r
    ? {
        ...
        fixed_shipping_fee: r.fixed_shipping_fee,
        priority: r.priority,
...
async function deleteRule(id: string) {
  try {
    await del(`/api/admin/v1/pricing/rules/${id}`);
    fetchRules();
  } catch (e) {
    console.error(e);
  }
...
async function deletePromo(id: string) {
  try {
    await del(`/api/admin/v1/pricing/promotions/${id}`);
    fetchPromos();
  } catch (e) {
    console.error(e);
  }
  ...
  <div class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-4 pt-2">
          <div class="flex justify-between items-center">
            <span class="text-sm text-[var(--n-text-color-3)]">
              {{ $t('page.pricing.promoCount', { count: promotions.length }) }}
            </span>
      ...
      <NTabPane name="calculator" :tab="$t('page.pricing.priceCalculator')">
        <NCard :title="$t('page.pricing.priceCalculator')" size="small" style="max-width: 480px" class="mt-2">
          <div class="flex flex-col gap-3">
            ...
            <NFormItem :label="$t('page.pricing.costPrice')">
              <NInputNumber v-model:value="calc.cost_price" :min="0" :step="0.01" style="width: 100%" />
            </NFormItem>
            <NFormItem :label="$t('common.region')">
              <NSelect v-model:value="calc.region" :options="regionOpts" />
            </NFormItem>
            <NFormItem :label="$t('page.pricing.overridePrice')">
              <NInputNumber v-model:value="calc.override_price" :min="0" :step="0.01" style="width: 100%" />
            </NFormItem>
    ...
    <NModal v-model:show="showRuleModal" preset="card" :title="$t('page.pricing.pricingRule')" style="width: 480px">
      <NForm :model="ruleForm" label-placement="left" label-width="140">
        ...
        <NFormItem :label="$t('page.pricing.markupMultiplier')">
          <NInputNumber v-model:value="ruleForm.markup_multiplier" :min="0" :step="0.01" style="width: 100%" />
        </NFormItem>
        <NFormItem :label="$t('page.pricing.fixedShippingFee')">
          <NInputNumber v-model:value="ruleForm.fixed_shipping_fee" :min="0" :step="0.01" style="width: 100%" />
        </NFormItem>
        <NFormItem :label="$t('common.priority')">
          <NInputNumber v-model:value="ruleForm.priority" style="width: 100%" />
        </NFormItem>
    ...
    <NModal v-model:show="showPromoModal" preset="card" :title="$t('page.pricing.promotions')" style="width: 480px">
      <NForm :model="promoForm" label-placement="left" label-width="140">
        ...
        <NFormItem :label="$t('common.regionsComma')">
          <NInput v-model:value="promoRegions" @update:value="updatePromoRegions" />
        </NFormItem>
        <NFormItem :label="$t('common.categoriesComma')">
          <NInput v-model:value="promoCategories" @update:value="updatePromoCats" />
        </NFormItem>
