...
async def get_site_profile(db: AsyncSession = Depends(get_db)) -> dict[str, object]:
    ...
    profile_raw["config"] = merge_for_response(profile_raw.get("config") or {})  # type: ignore[arg-type]
