...
> **核心原则**：快速迭代优先。能用非浏览器手段自查的绝不派浏览器；必须真实浏览器确认的才派 browser-agent；能由用户实测的优先交给用户，避免多浏览器标签、缓存、登录态等环境问题反复绕路。

### 11.0 标准开发流程（总览）

**触发条件**：每次 Forge 功能开发或修复任务。

**执行步骤**：

1. **切分支**：先 `git` 归集挂起分支改动并提交到对应分支（勿误落 dev），再从 dev 切 feature 分支
2. **实现**：按任务完成代码实现（后端 api/repo/service/model + 必要 migration，容器内执行并先备份）
3. **提交**：提交信息遵循 Conventional Commits，pre-commit hook（提交文件级 ruff/mypy）自动把关，秒级通过；存量债不影响日常提交
4. **重建**：按需重建对应容器（admin 无 HMR，必须 `build --no-cache` + `--force-recreate`）
5. **e2e**：重建/重启后必须完成端到端验证（API 链路、数据链路、需求逐条核对），禁止仅凭"源码存在"宣布成功

**说明**：本节为流程总览，各环节细则见 §1（版本管理）、§10（重建后端到端验证）、§13（代码质量与安全检查）、§15（版本发布与提交规范）。

### 11.1 浏览器验证按必要性判断，不默认派发

**触发条件**：任何 Forge 开发迭代的验证环节。
...
3. **派发时强制要求**：明确要求 browser-agent 核对加载的 JS hash 与预期构建一致（如 `resources-C7DZJ4t9.js`），从源头排除浏览器缓存干扰
4. **能交给用户验证的交给用户**：完成代码修改与容器部署后，优先向用户说明验证点（访问地址、操作步骤、预期结果），由用户实测并反馈截图或报错文本
5. **非开发验证需求不受限**：查资料、访问网站、网页交互等日常需求仍按原路由派发 browser-agent

**反例（2026-08-17）**：资源管理 checkbox + 删除报错修复后，先派 browser-agent 验证发现旧逻辑，排查才发现首次 admin 构建未真正执行 builder 阶段；第二轮 browser-agent 又误报"NMessageProvider 缺失"，多花两轮环境排查。若直接核对容器产物 + 交给用户实测，可省去大量绕路。

### 11.2 构建产物强制核对

**触发条件**：每次执行 `build --no-cache` 构建 admin 等镜像之后。

**执行步骤**：

1. **禁止以 "Build successful" 作为完成依据**——首次 build 可能缓存命中 builder 阶段，`pnpm build` 并未真正执行
2. **核对容器内产物**：`podman exec <容器> ls -la /usr/share/nginx/html/assets/<产物>.js`，确认时间戳/hash 晚于源码修改时刻
3. **grep 关键逻辑**：`podman exec <容器> cat .../assets/<产物>.js | grep <修复特征字符串>`，确认修复已进入产物
4. 核对通过后才进入验证环节

**反例（2026-08-17）**：`build --no-cache admin` 输出成功但容器内 JS 仍是旧时间戳（19:45 vs 22:40），未核对即派发验证，浪费一整轮。
...
**反例（2026-08-17）**：browser-agent 称"新构建缺 NMessageProvider 导致提示不渲染"，实际编译产物 `index--cbXpLMw.js` 中 AppProvider 正常挂载 MessageProvider，属误报。
...
> **核心原则**：方案质量以行业标准为锚。任何架构 / 重构 / 规范 / 交互设计方案（如错误契约、目录规范、部署流程等）必须先对照权威实践与成熟产品做对标评审，再进入实施；禁止按直觉直接出方案。

### 19.1 触发条件

- 输出任何涉及设计决策的方案（前后端架构、接口契约、错误码/响应体规范、命名与目录规范、流程 SOP、组件设计等）
- 用户要求"对比行业 / 看看成熟产品怎么做 / 方案是否合理"等评审类诉求
- 上一版方案被质疑或需要收敛时（如"别这里改一下那里改一下"）

### 19.2 执行步骤（对标评审六步）

1. **行业对标**：先找权威基准再谈方案——
   - 协议/规范类：优先 RFC（如错误响应 RFC 9457）、Google AIP、W3C 等正式标准
   - 产品惯例类：Stripe / AWS / Shopify / GitHub 等公开 API 文档或同规模成熟开源项目
   - 禁止凭记忆杜撰他人产品行为；引用具体结构时必须准确
2. **亮点高亮**：明确列出本方案与行业主线一致 / 优于行业惯例的点。设计评审不是批斗会，符合主流的部分要显式认可，避免为改而改
3. **差距补齐**：以对比表逐项列出差异（方案 vs 行业），识别缺项（字段级错误、幂等、语义边界、可扩展性等），逐项给出修正；对不采纳的差异说明原因
4. **合理性评估**：说明关键取舍的 why——哪些是行业标准做法、哪些是因团队规模 / 项目阶段 / 兼容成本主动简化；标注简化是否留了演进路径
5. **标准化评估**：字段语义无歧义、可扩展（新增场景不散落改动）、可自动化校验（lint/CI/pre-commit）、有文档承载
6. **结论先行**：先给"方向成立与否"结论 → 修订项 → 待用户决策项；修订后的方案才算定稿

### 19.3 输出结构要求

- 必须使用**对比表**（方案 vs 行业基准），逐行给差距与修正
- 必须同时出现三个评价面：亮点（高亮）、差距（修正）、合理性/标准化结论
- 方案定稿前标注待用户决策项，未经确认不进入实施

**反例（2026-09-03）**：错误提示优化第一版直接输出自研 `{code, detail}` 结构，未对照 RFC 9457 / Stripe / Google AIP-193，缺字段级校验错误、message 语义未定义、双字段兼容埋债。经对标评审后收敛为标准两层结构 `{code, message, status, errors[]}` + type/code 双层错误码模型。
