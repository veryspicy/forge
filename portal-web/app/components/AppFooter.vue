        ...
        <div v-if="socialItems.length" class="flex items-center gap-3">
          ...
          >
            <svg
              ...
              role="img"
              aria-hidden="true"
              v-html="SOCIAL_ICONS[item.platform] || ''"
            />
          </a>
        </div>
...
// 社交平台内联 SVG path（24x24 viewBox，fill=currentColor 随文字色变化）
...
const socialItems = computed(() =>
  (profile.value.footer.social || [])
    .filter((s: any) => s && s.enabled && s.url && s.platform && SOCIAL_ICONS[s.platform])
